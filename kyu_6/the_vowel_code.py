...
# given string with numbers according to the
# following pattern:
#
# a -> 1
# e -> 2
...
# to turn the numbers back into vowels according to
...
def encode(st: str) -> str:
    for letter in PATTERN:
        st = re.sub(letter, PATTERN[letter], st)
    return st


def decode(st):
    for letter in PATTERN:
        st = re.sub(PATTERN[letter], letter, st)
    return st
